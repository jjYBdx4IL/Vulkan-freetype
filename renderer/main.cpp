    ...
    void* userData)
{
    if (flags & VK_DEBUG_REPORT_ERROR_BIT_EXT) {
        DBG("validation error:  %s\n", msg);
    }
    else {
        DBG("validation layer: %s\n", msg);
    }
...
static void pick_physical_device(fd_Render *r)
{
    ...
    physical_devices = (VkPhysicalDevice *) malloc(sizeof(VkPhysicalDevice) * physical_device_count);
...
static void find_queue_families(fd_Render *r)
{
    ...
    properties = (VkQueueFamilyProperties *) malloc(sizeof(VkQueueFamilyProperties) * family_count);
...
static void create_image_views(fd_Render *r)
{
    ...
    for (uint32_t i = 0; i < r->image_count; i++)
    {
        VkImageViewCreateInfo ci = {
            ...
            .format = r->format,
            .components = { VkComponentSwizzle::VK_COMPONENT_SWIZZLE_IDENTITY },
            .subresourceRange = {
...
static void create_render_pass(fd_Render *r)
{
...
{
    r->framebuffers = (VkFramebuffer*) malloc(sizeof(VkFramebuffer) * r->image_count);
...
static void create_command_pool(fd_Render *r)
{
...
static void begin_text(fd_Render *r)
{
...
static void append_text(fd_Render *r, float x, float y, float scale, const char *text)
{
    while (*text)
    {
        ...
        fd_HostGlyphInfo *gi = &r->glyph_infos[glyph_index];
        fd_GlyphInstance *inst = &r->glyph_instances[r->glyph_instance_count];
...
static void record_current_command_buffer(fd_Render *r)
{
    //printf("%d\n", r->ring_buffer_index);
    VkCommandBuffer cmd_buf = r->command_buffers[r->ring_buffer_index];
    ...
    VkClearValue clear_value = { 0.1f, 0.2f, 0.3f, 1.0f };
...
static void record_command_buffers(fd_Render *r)
{
...
{
    r->command_buffers = (VkCommandBuffer*) malloc(sizeof(VkCommandBuffer) * r->ring_buffer_count);
...
{
    r->command_buffer_fences = (VkFence*) malloc(sizeof(VkFence) * r->ring_buffer_count);
...
static void create_layout(fd_Render *r)
{
...
static uint32_t find_memory_type(fd_Render *r, uint32_t type_bits, VkMemoryPropertyFlags flags)
{
...
static VkDeviceMemory alloc_required_memory(fd_Render *r, VkMemoryRequirements *req, VkMemoryPropertyFlags flags)
{
...
static void create_buffer_with_memory(
    fd_Render *r, VkBufferCreateInfo *ci, VkMemoryPropertyFlags flags,
...
VkCommandBuffer begin_one_time_cmdbuf(fd_Render *r)
{
...
void end_one_time_cmdbuf(fd_Render *r, VkCommandBuffer cmd_buffer)
{
...
static void copy_buffer(fd_Render *r, VkBuffer src_buffer, VkBuffer dst_buffer, VkDeviceSize size)
{
...
static void stage_buffer(fd_Render *r, VkBuffer buffer, void *data, size_t size)
{
...
static void create_instance_buffer(fd_Render *r)
{
...
static void create_descriptor_pool(fd_Render *r)
{
...
static void create_descriptor_set(fd_Render *r)
{
...
static void create_pipeline(fd_Render *r)
{
#ifdef _WIN32
    // assuming that the project .sln is located at REPO-ROOT/build and MSVC executes the debug executable relative to that dir.
    VkShaderModule vs_font = load_shader_module(r->device, "../renderer/shaders/font.vert.spv");
    VkShaderModule fs_font = load_shader_module(r->device, "../renderer/shaders/font.frag.spv");
#else
    ...
    VkVertexInputBindingDescription vertex_input_binding =
        { 0, sizeof(fd_GlyphInstance), VK_VERTEX_INPUT_RATE_INSTANCE, };
...
static void create_swap_chain_objects(fd_Render *r)
{
...
static void destroy_swap_chain_objects(fd_Render *r)
{
...
static void recreate_swap_chain(fd_Render *r)
{
    vkDeviceWaitIdle(r->device);
...
static void update(fd_Render *r)
{
    r->frame++;
    r->fps_temp++;
    ...
    vec2 mouse_delta;
    vec2_sub(mouse_delta, r->old_mouse_pos, r->mouse_pos);

    if (glfwGetMouseButton(r->window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
    {
        vec2_scale_(mouse_delta, 1.0f / r->canvas_scale);
        vec2_add_(r->canvas_offset, mouse_delta);
    }

    vec2_dup(r->old_mouse_pos, r->mouse_pos);

    vec2 swapchain_extent = {
        (float)r->swapchain_extent.width,
        (float)r->swapchain_extent.height
    };

    vec2 old_size;
    vec2_scale(old_size, swapchain_extent, 1.0f / r->canvas_scale);

    if (r->canvas_scale != r->target_canvas_scale)
    {
        r->canvas_scale = lerpf(r->canvas_scale, r->target_canvas_scale, r->delta_time * 30.0f);

        vec2 new_size;
        vec2_scale(new_size, swapchain_extent, 1.0f / r->canvas_scale);

        vec2 tmp;
        vec2_sub(tmp, old_size, new_size);

        tmp[0] *= r->mouse_pos[0] / swapchain_extent[0];
        tmp[1] *= r->mouse_pos[1] / swapchain_extent[1];

        vec2_add_(r->canvas_offset, tmp);
    }
}

static void render_frame(fd_Render *r)
{
...
static void on_window_resized(GLFWwindow* window, int width, int height)
{
    ...
    recreate_swap_chain((fd_Render *)(glfwGetWindowUserPointer(window)));
}
...
{
    fd_Render *render = (fd_Render *)(glfwGetWindowUserPointer(window));
...
{
#ifdef _WIN32
    const char* font_face = "C:\\Windows\\Fonts\\times.ttf";
#else
    const char* font_face = "times.ttf";
#endif

    glfwInit();

    glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
    // keep window hidden until everything is set up
