...
static void create_swap_chain(fd_Render *r)
{
	...
	else
	{
		...
		uint32_t w, h;
		w = max(capabilities.minImageExtent.width, min(capabilities.maxImageExtent.width, (uint32_t)window_w));
		h = max(capabilities.minImageExtent.height, min(capabilities.maxImageExtent.height, (uint32_t)window_h));
